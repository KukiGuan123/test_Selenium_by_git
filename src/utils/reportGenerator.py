        ...
        for (f, s), steps in summary.items():
            ...
            for step in steps:
                if step["result"] == "Fail":
                    ...
                    reason = step["error"]
                    break
                elif step["result"] == "Skip":
                    res = "Skip"
            self.ws_sum.append([f, s, res, fail_step, reason])
            if res == "Pass":
                self.ws_sum[f"C{self.ws_sum.max_row}"].font = Font(bold=True, color="008000")
            elif res == "Fail":
                self.ws_sum[f"C{self.ws_sum.max_row}"].font = Font(bold=True, color="FF0000")
            else:
                self.ws_sum[f"C{self.ws_sum.max_row}"].font = Font(bold=True, color="FFA500")
                ...
                for idx, step in enumerate(steps):
                    ...
                    try:
                        ...
                        if step["result"] == "Pass":
                            ...
                            res_run.font.color.rgb = RGBColor(255, 165, 0)
                    except:
                        pass
